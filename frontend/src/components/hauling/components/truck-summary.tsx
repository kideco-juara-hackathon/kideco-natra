...
import { getRouteOptions, loadingPoints, type Truck as TruckData } from "@/data/hauling-screens";
...
export function TruckSummary({
  ...
  // Calculate distance
  const distanceMeter = (recommendedRoute.emptyRoute.distanceMeter ?? 0) + (recommendedRoute.loadedRoute.distanceMeter ?? 0);
  const distanceKm = distanceMeter > 0 ? (distanceMeter / 1000).toFixed(1).replace(".", ",") : "4,2";

  // Circular progress variables
  const radius = 15;
  const strokeWidth = 3;
  const circumference = 2 * Math.PI * radius;
  const strokeDashoffset = circumference - (truck.healthScore / 100) * circumference;

  // Health quality label
  const healthLabel = truck.healthScore >= 85 ? "Baik" : truck.healthScore >= 70 ? "Cukup" : "Kurang";
  const healthColor = truck.healthScore >= 85 
    ? "text-emerald-600 dark:text-emerald-400" 
    : truck.healthScore >= 70 
      ? "text-amber-500" 
      : "text-red-500";
  const ringColor = truck.healthScore >= 85 
    ? "stroke-emerald-500" 
    : truck.healthScore >= 70 
      ? "stroke-amber-500" 
      : "stroke-red-500";
    ...
    <div className="flex flex-col gap-4 text-[var(--text-default)]">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="rounded-xl border border-[var(--border-default)] p-3 bg-[var(--bg-surface)] flex items-center justify-between gap-2">
          <div className="min-w-0">
            ...
            <p className={`text-[12px] font-bold mt-1 leading-none ${healthColor}`}>
              {healthLabel}
            </p>
          </div>
      ...
      <div className="rounded-xl border border-[var(--border-default)] p-4 bg-[var(--bg-surface)] flex flex-col gap-3">
        <div>
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            <span className="text-[14px] font-bold text-[var(--text-default)]">Dispatch Point</span>
          </div>
        </div>

        <div className="border-t border-[var(--border-subtle)] pt-2.5 flex items-center justify-between">
          <span className="text-[11px] font-semibold text-[var(--text-muted)]">Node saat ini</span>
      ...
      <div className="rounded-xl border border-[var(--border-default)] p-4 bg-[var(--bg-surface)] flex flex-col gap-3">
        <div className="flex items-center gap-1.5 text-amber-500">
          <Sparkles className="size-4 fill-amber-500" />
        ...
        <p className="text-body-sm text-[var(--text-muted)] leading-relaxed font-medium">
          Berdasarkan kondisi jalan, antrian, dan jarak, kami merekomendasikan rute ke{" "}
          <strong className="font-bold text-[var(--text-default)]">
            {loadingPointName(recommendedRoute.loadingPointId)}
          </strong>{" "}
          melalui <strong className="font-bold text-[var(--text-default)]">Rute {recommendedRoute.id}</strong>.
        </p>

        {/* 3-Column Metrics */}
        <div className="border-t border-[var(--border-subtle)] pt-2.5 grid grid-cols-3 gap-2">
          <div>
            <p className="text-[11px] font-semibold text-[var(--text-muted)] leading-none">Jarak</p>
            <p className="mt-1.5 text-[12px] font-bold text-[var(--text-default)] truncate">
              {distanceKm} km
            </p>
          </div>
          <div>
            <p className="text-[11px] font-semibold text-[var(--text-muted)] leading-none">ETA</p>
            <p className="mt-1.5 text-[12px] font-bold text-[var(--text-default)] truncate">
              {recommendedRoute.etaMin} min
            </p>
          </div>
          <div>
            <p className="text-[11px] font-semibold text-[var(--text-muted)] leading-none">Est. Muatan</p>
            <p className="mt-1.5 text-[12px] font-bold text-[var(--text-default)] truncate">
